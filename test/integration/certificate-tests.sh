...
# sha256sum is different due to inserted newline if printed to stdout
test "getcertificate the same but absolute path" assert.Success "azmi getcertificate --certificate ${KV_RO}/certificates/readThisCertificatePfxFormat -f /var/tmp/download.txt --identity $identity && sha256sum /var/tmp/download.txt | grep 1284f98a73095f554f0dd50b640618520183f11a7961d609dd05c963e35e5056"
